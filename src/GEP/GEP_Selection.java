...
class Elitism{
	// Returns the fittest element
	public static Karva selectFittest(ArrayList<Karva> population){
		
		Iterator<Karva> iterator = population.iterator();
		Karva fittest = iterator.next();
		
		while(iterator.hasNext()){
			
			Karva current = iterator.next();
...
class RouletteWheel{
	...
	public static Karva selectOne(ArrayList<Karva> elements){
		...
		while(iterator.hasNext())
			// return the element corresponding to the current slice if the random number is bigger than the current slice limit, but wasn't bigger than the previous
			if(random<iterator.next())
