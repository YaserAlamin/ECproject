...
class GeneExpressionAlghorithm{
	 ...
	 * @return 
	 */
	public static void gep(int SEL_method, int N, int l, int s, double pm, double pc, double r, int g){
		...
		double max;// = Karva.fitness_max(population).getFitness();
		double avg;// = Karva.fitness_average(population);
		double min;// = Karva.fitness_min(population).getFitness();
		double stdDeviation;// = Karva.fitness_stdDeviation(population);


		//System.out.println("0: " + df.format(max) + " " + df.format(avg) + " " + df.format(min));

		//for all the g iterations
		for(int i=0; i<g;i++){
			population = gepIteration(SEL_method, population, N, l, s, pm, pc, r);
		...
		max = Karva.fitness_max(population).getFitness();
		avg = Karva.fitness_average(population);
		min = Karva.fitness_min(population).getFitness();
		stdDeviation = Karva.fitness_stdDeviation(population);
	...
	//Order of methods is different from GEP paper
	private static ArrayList<Karva> gepIteration(int SEL_method, ArrayList<Karva> population, int N, int l, int s, double pm, double pc, double r){
		
		System.out.println("Population:");
		Karva.printListOnePerLine(population);
		
		//ELITISM
		Karva fittest = Elitism.selectFittest(population);
		...
		//Put the fittest element on a random position of the list
		Random dice = new Random();
		selected.set(dice.nextInt(selected.size()-1), fittest);
				
		//MUTATION
		//apply mutation to every element
		for(int i=0;i<selected.size();i++)				
			selected.set(i, Mutation.mutate(selected.get(i), pm));
		...
		ArrayList<Karva> population_new0 = new ArrayList<Karva>();
		...
		for(int i=0; i<selected.size()/2; i++){
			if(RandomGenerator.generate() < pc){
				...
				offspring = CrossOver.onePointCrossOver(selected.get(i*2).getRepresentation(), selected.get(i*2+1).getRepresentation());
				
				Karva child0 = new Karva(offspring.get(0));
				Karva child1 = new Karva(offspring.get(1));
				population_new0.add(child0);
				population_new0.add(child1);
			}
			else{
				population_new0.add(selected.get(i*2));
				population_new0.add(selected.get(i*2+1));
			}
		}
		
		//apply One Point Crossover to every two elements of the selected list
		ArrayList<Karva> population_new1 = new ArrayList<Karva>();

		// for every 2 elements of the selected list, according to crossover probability
		// cross over and add to the new population
		for(int i=0; i<population_new0.size()/2; i++){
			if(RandomGenerator.generate() < pc){
				ArrayList<String> offspring = new ArrayList<String>();
				offspring = CrossOver.twoPointCrossOver(population_new0.get(i*2).getRepresentation(), population_new0.get(i*2+1).getRepresentation());
				
				Karva child0 = new Karva(offspring.get(0));
				Karva child1 = new Karva(offspring.get(1));
				population_new1.add(child0);
				population_new1.add(child1);
			}
			else{
				population_new1.add(selected.get(i*2));
				population_new1.add(selected.get(i*2+1));
			}
		}
		...
		// generates testing Sequences and use them to calculate the fitness for each element
		ArrayList<Sequence> testingSequences = Sequence.generateList(15, 30);
		Karva.populationFitness(population_new1, testingSequences);
		
		ArrayList<Karva> population_final = Replication.replace(population, population_new1, r);
